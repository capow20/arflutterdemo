...
namespace Arway
{
    public class ItemClickController : MonoBehaviour
    {
        ...
        public void GetMapId(string mapInfo)
        {
            Debug.Log("**********MapInfo passed to GetMapId: " + mapInfo + "**********");
            string map_id = mapInfo.Split('-')[0];//gameObject.transform.name;
            string map_name = mapInfo.Split('-')[1];//gameObject.transform.Find("name").GetComponent<Text>().text;

            StartCoroutine(UpdateMapDetails(map_id, map_name));

            
            Manager.SendMessageToFlutter("Ready to Localize");
        }
